...
[W0, xx, t] = tRKDG0(e, T, a, int_l, int_p, pp,'off');
[W1, ~, ~] = tRKDG1(e, T, a, int_l, int_p, pp,'off');
[W2, ~, ~] = tRKDG2(e, T, a, int_l, int_p, pp,'off');

% /~~~~~~~~~~~~~~~~~~~~~~~~~ GRAFICKÉ VÝSTUPY ~~~~~~~~~~~~~~~~~~~~~~~~~~\ %
% subplot(2,1,1)
    plot(xx,W0(:,1),'-m',xx,W0(:,end),'-b',xx,W1(:,end),'-k',xx,W2(:,end),'-r');
    legend({'exact','$k = 0$','$k = 1$','$k = 2$'},'Interpreter','latex');
    xlabel('$x$', 'Interpreter', 'latex'); 
    ylabel('$w(x,1)$', 'Interpreter', 'latex'); 
    grid on;
%     set(gca, 'TickLabelInterpreter', 'latex')
% subplot(2,1,2)
%     surf(xx,t,W0', 'LineStyle', 'none')
%     xlabel('$x$', 'Interpreter', 'latex'); 
