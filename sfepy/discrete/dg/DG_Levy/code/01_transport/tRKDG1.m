...
fprintf('RKDG1 Bude provedeno %d casovych kroku\n\n', ceil(T / dt));
% \~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~/ %
...
% int0 = Gauss(x1, (x2 - x1), @(x) psi0(x) .* psi_x(x));
% int1 = Gauss(x1, (x2 - x1), @(x) psi1(x) .* psi_x(x));

al = (x2 - x1) / (2 * l + 1); % gives 3 in case of linear approx and 1 in constant approx
...
Lal = a * (c(1) * int0 + c(2) * int1) + tok_minus * (- 1) ^ l - tok_plus;
L = Lal / al;  % A\(f(x_k) - f(x_k+1)) i.e. A^(-1) * (f(x_k) - f(x_k+1))
end