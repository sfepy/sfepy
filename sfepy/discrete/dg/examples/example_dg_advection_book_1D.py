...
# Setup output names
domain_name = "domain_book_1D"
output_folder = "output/adv_book_1D/"
...
left_fix_u = EssentialBC('left_fix_u', left,
                         {'u.all': Function('bc_fun', lambda ts, coor, bc, problem, **kwargs:
                                                        left_sin(ts.time))})
#
...
pb.save_state(output_folder + "/" + domain_name + "_start.vtk", state=state0)
...
pb.set_solver(tss)
state_end = pb.solve()
pb.save_state(output_folder + "/" + domain_name + "_end.vtk", state=state_end)


#----------
#| Plot 1D|
#----------
lmesh, u = load_1D_vtks(output_folder, domain_name, order=approx_order)
...
coors, u_end = load_state_1D_vtk(output_folder + "/" + domain_name + "_end.vtk", order=approx_order)
