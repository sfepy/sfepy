...
AdvFluxT = AdvectionDGFluxTerm("adv_lf_flux(alpha.val, a.val, v, u)", "a.val, v, u[-1]",
...
dt = dx ** 2 / diffusion_tensor * CFL * order_corr
...
output(f"Courant number c = max(abs(D)) * dt/dx^2 = {diffusion_tensor * dtdx/dx}")
output("------------------------------------------")
