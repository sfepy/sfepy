...
def build_attrs_string(conf, attrs=("Cw", "diffusion_coef", "dt", "CFL"),
    ...
    """
    Builds string from intersection of attributes list attrs and conf attributes,
    removes "." !
    :param conf:
    :param attrs:
    :param sep:
    ...
    for attr in attrs:
        ...
        if attr_val is not None:
            form += sep + attr + "{}"
            attr_vals += [attr_val]
    res = form.format(*attr_vals) if not remove_dots else form.format(*attr_vals).replace(".", "")
...
def plot_conv_results(base_output_folder, conf, err_df, plot_title_attrs=None, save=False):
    ...
    fig_sup_title += build_attrs_string(conf, plot_title_attrs, sep=", ", remove_dots=False)
