...
Depending on Python installation and OS used, replacing ``python`` by ``python3`` might be required
in all the commands below (e.g. in :ref:`compilation`) in order to use Python 3. However we strictly recommend to use *Python Virtual Environments* (see `Python virtualenv`_ or `Conda Environments`_) for these purposes.

The far easiest way we recommend to use *SfePy* on any supported platform is to install `Anaconda`_ Python distribution
(see `Multi-platform Python Distributions Notes`_) and follow `Generic SfePy Installation Instructions`_.
...
Make sure the dependencies of those packages are also installed - `igakit`_ reguires FORTRAN compiler,
...
- If `doxygen` is installed, the documentation of data structures and functions
  ...
  doxygendocs``. Then see ``doc/html/index.html`` for results.
...
On Windows we highly recommend a free version of commercials multi-platform scientific
Python distributions `Anaconda`_ (from `Continuum Analytics`_) or `Enthought Canopy`_
(formerly Enthought Python Distribution). See `Multi-platform Python Distributions Notes`_
for more details.

A completely free scientific-oriented distributions `WinPython`_ or `Python(x,y)`_
can be used, but some compatibility issues or missing packages may expected (Instructions
for installing `Python(x,y)`_ can be found in `Windows Using Python(x,y)`_).

*SfePy* can be used without any installation by running requested scripts from the
...
Generic SfePy Installation Instructions
---------------------------------------
...
(We highly recommend this scientific Python distribution. Currently tested with Python 2.7/3.6 64-Bit
on Ubuntu 14.04 LTS, Windows 8.1 and MacOS 10.10+.)
...
To build *SfePy* extension modules, included `mingw-w32/64`_ compiler tools should work fine. If you encounter any
problems, we recommend to use Microsoft `Visual C++ Build Tools`_ instead (see `Anaconda FAQ`_).
...
Now follow the `Generic SfePy Installation Instructions`_ above.
