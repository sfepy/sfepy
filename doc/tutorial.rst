...
Finite Element Method (FEM) is the numerical method for solving Partial
Differential Equations (PDE). FEM was developed in the middle of XX century and
...
fluid dynamics and other. FEM is based on very elegant mathematical theory of
weak solution of PDE. In this section we will briefly discuss basic ideas
underlying FEM.

Strong form of Poisson's equation and its integration
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Let us start our discussion about FEM with strong form of Poisson's
equation
...
were Dirichlet boundary conditions are given, :math:`\Gamma_N` is the part of
the boundary where Neumann boundary conditions are given, :math:`T(x)` is the
unknown function to be found, :math:`f(x), u(x), g(x)` are the known functions.

FEM method is based on a weak formulation. The weak form of the
equation :eq:`eq_spoisson` is

.. math::
    \int\limits_{\Omega} (\Delta T - f) \cdot s \, \mathrm{d}\Omega = 0,

where :math:`s` is the **test** function. Integrating this equation by parts
...
The surface integral term can be splitted into the two integrals, one over the
...
with it without applying the boundary conditions. So it is a time to talk about
the boundary conditions.

Dirichlet Boundary Conditions
"""""""""""""""""""""""""""""

On Dirichlet part of the surface we have two restrictions. One is the Dirichlet
...
In other words the unknown function :math:`T` must be continues together with
its gradient in the domain. In contrast the test function :math:`s` must be 
also continues together with its gradient in the domain but
on the surface :math:`\Gamma_D` it is defined to be zero.

With this requirements the integral term over Dirichlet part of the surface
...
**Essential Boundary Conditions**. These conditions are not a part of weak
...
The Neumann boundary conditions are the known flux
...
It is planned to have an example of the discretization base on the Poisson's
...
In the next section we will discuss how to define all these things in the SfePy.
